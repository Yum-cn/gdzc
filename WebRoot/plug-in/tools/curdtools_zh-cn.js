...
$(function () {
    $("#selectAll").click(function () {//全选、反选
        $(".layui-table :checkbox").each(function () {
        	if($("#selectAll").attr("checked")==undefined){
...
function deleteNewStyleALLSelect(title,url,gname) {
    var ids = [];
    ...
    if (ids.length > 0) {
    	$.dialog.setting.zIndex = getzIndex(true);
    	$.dialog.confirm('你确定删除这些数据吗?', function(r) {
		  // if (r) {
				$.ajax({
					url : url,
					type : 'post',
...
function deleteDialog(title, url,id) {
	$.dialog.setting.zIndex = getzIndex(true);
	$.dialog.confirm('你确定删除这条数据吗?', function(r) {
	  // if (r) {
			$.ajax({
				url : url,
				type : 'post',
